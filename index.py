...
def is_valid_license_plate(before_data, after_data):
    ...
    # limit api calls to plate checker api by only checking the best score for an event
    if(not after_license_plate_attribute[0]['score'] == after_data['attributes']['license_plate'] or not after_data['attributes']['license_plate'] == before_license_score or not before_license_score == 0 and after_data['id'] in CURRENT_EVENTS):
        _LOGGER.debug(f"duplicated snapshot from Frigate as license plate score from before greater than after: {after_data['attributes']['license_plate']} {after_data['id']}")
        return False
...
def on_message(client, userdata, message):
    ...
    if type == 'end' and after_data['id'] in CURRENT_EVENTS:
        # _LOGGER.debug(f"Clearing event: {frigate_event_id}")
        CURRENT_EVENTS.remove(frigate_event_id)
